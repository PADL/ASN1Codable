...
extension ASN1DecoderImpl.SingleValueContainer {
    ...
    private func decode<T>(
        ...
        } else if let type = type as? any OptionSet.Type {
            value = try self.decodeOptionSetValue(type, from: object) as! T
        } else {
    ...
    private func decodeOptionSetValue<T>(
        _ type: T.Type,
        from object: ASN1Object,
        verifiedTag: Bool = false
    ) throws -> T where T: OptionSet {
