...
extension HeimASN1Translator {
    ...
    fileprivate func apply(_ block: (_ type: HeimASN1TypeDef, _ stop: inout Bool) -> ()) {
        var stop = false
...
extension HeimASN1Translator {
    private func type(_ someType: HeimASN1TypeDef, isMemberOf containingType: HeimASN1TypeDef) -> Bool {
        return containingType.members?.contains {
            return $0.typeDefValue?.tTypeTypeRefValue == someType.name
        } ?? false
    }
    
    private func typeNeedsTransitiveHashableConformance(_ someType: HeimASN1TypeDef) -> Bool {
        return someType.members?.contains {
            if let typeDefValue = $0.typeDefValue {
                return self.type(someType, isMemberOf: typeDefValue) &&
                    self.typeNeedsHashableConformance(typeDefValue)
            } else {
                return false
            }
        } ?? false
    ...
    func typeNeedsHashableConformance(_ someType: HeimASN1TypeDef) -> Bool {
        if someType.needsHashableConformance ?? false {
            return true
        }
        
        var needsHashableConformance: Bool = false

        self.apply({ typeDef, stop in
            if let typeRefContainedBySetOrSetOf = typeDef.typeContainedBySetOrSetOf?.typeRefValue {
                self.typeDefsByName[typeRefContainedBySetOrSetOf]?.needsHashableConformance = true
                
                if typeRefContainedBySetOrSetOf == someType.name {
                    needsHashableConformance = true
                }
            } else if typeNeedsTransitiveHashableConformance(someType) {
                needsHashableConformance = true
            }

            if needsHashableConformance {
                stop = true
            }
        })
        
        someType.needsHashableConformance = needsHashableConformance
