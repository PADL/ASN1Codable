...
extension ASN1CodableTests {
    func test_encode_BIT_STRING() {
        // FIXME: add more tests when we support not rounding to octet boundary
        self.test_encode(BitString([0xFF]), encodedAs: Data([0x03, 0x02, 0x00, 0xFF]))
    }

    func test_encode_BMPString() {
        let greekCapitalLetterSigma = BMPString(wrappedValue: "Σ")
        self.test_encode(greekCapitalLetterSigma, encodedAs: Data([0x1E, 0x02, 0x03, 0xA3]))
    }

    func test_encode_BOOLEAN() {
        self.test_encode(true, encodedAs: Data([0x01, 0x01, 0xFF]))
        self.test_encode(false, encodedAs: Data([0x01, 0x01, 0x00]))
    }
    ...
    func test_encode_GeneralString() {
        // FIXME: check non-ASCII characters
        @GeneralString var mixedChars = "ABC"
        self.test_encode(_mixedChars, encodedAs: Data([0x1B, 0x03, 0x41, 0x42, 0x43]))
    }
    ...
    func test_encode_GeneralizedTime() {
        ...
        self.test_encode(GeneralizedTime(wrappedValue: dateComponents.date!),
                         encodedAs: Data([0x18, 0x0F, 0x31, 0x39, 0x39, 0x36, 0x30, 0x34,
                                          0x31, 0x35, 0x32, 0x30, 0x33, 0x30, 0x30, 0x30,
                                          0x5A]))
    }

    func test_encode_IA5String() {
        @IA5String var ia5 = "ABCD EFGH"
        self.test_encode(_ia5, encodedAs: Data([0x16, 0x09, 0x41, 0x42, 0x43, 0x44, 0x20, 0x45,
                                                0x46, 0x47, 0x48]))
    }
    ...
    func test_encode_OBJECT_IDENTIFIER() {
        self.test_encode(ASN1Kit.ObjectIdentifier(rawValue: "1.0")!,
                         encodedAs: Data([0x6, 0x1, 0x28]))
        self.test_encode(ASN1Kit.ObjectIdentifier(rawValue: "2.100.3")!,
    ...
    func test_UTCTime() {
        let dateComponents = DateComponents(calendar: Calendar(identifier: .gregorian),
                                            timeZone: TimeZone(secondsFromGMT: 0),
                                            year: 1996, month: 04, day: 15, hour: 20, minute: 30)
        self.test_encode(UTCTime(wrappedValue: dateComponents.date!),
                         encodedAs: Data([0x17, 0x0D, 0x39, 0x36, 0x30, 0x34, 0x31, 0x35,
                                          0x32, 0x30, 0x33, 0x30, 0x30, 0x30, 0x5A]))
    }

    func test_encode_UTF8String() {
        @UTF8String var utf8 = "公的個人認証サービス"
        self.test_encode(_utf8, encodedAs: Data([0x0C, 0x1E, 0xE5, 0x85, 0xAC, 0xE7, 0x9A, 0x84,
